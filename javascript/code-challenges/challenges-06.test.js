...
const toLastNames = people => {
  const newObj = people.map(person => `${person.firstName} ${person.lastName}`);
  return newObj;
};
...
const addPurchases = (arr) => {
  const newArr = arr.reduce((total, current) => total + current.purchasePrice, 0);
  return newArr;
};
...
const countNumberOfElements = (arr) => {
  const newArr = arr.reduce((count, item) => count + 1, 0);
  return newArr;
};
...
const returnNames = (arr) => {
  const newArr = arr.reduce((names, character) => {
